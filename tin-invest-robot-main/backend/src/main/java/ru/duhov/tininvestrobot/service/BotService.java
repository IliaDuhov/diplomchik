package ru.duhov.tininvestrobot.service;

import lombok.RequiredArgsConstructor;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.HttpStatus;
import org.springframework.stereotype.Service;
import ru.duhov.tininvestrobot.controller.exception.RestRuntimeException;
import ru.duhov.tininvestrobot.domain.*;
import ru.duhov.tininvestrobot.domain.*;
import ru.duhov.tininvestrobot.dto.BotConfig;
import ru.duhov.tininvestrobot.dto.BotEnv;
import ru.duhov.tininvestrobot.dto.BotPreview;
import ru.duhov.tininvestrobot.dto.StatisticDto;
import ru.duhov.tininvestrobot.repository.InstrumentRepository;
import ru.duhov.tininvestrobot.repository.InvestBotRepository;
import ru.duhov.tininvestrobot.service.candle.CandleService;
import ru.duhov.tininvestrobot.service.candle.CandleSteamsHolder;
import ru.duhov.tininvestrobot.service.notification.TelegramNotification;
import ru.duhov.tininvestrobot.service.order.*;
import ru.duhov.tininvestrobot.service.order.*;
import ru.duhov.tininvestrobot.simulator.Simulator;
...
public class BotService {
    ...
    public BotPreview createBotSimulation(BotConfig config, Instant startSimulation) {
        ...
        if (config.getTelegramBotChatId() != null) {
            telegramNotification.ifPresent(it ->
                    it.addObserver(bot.getState().getUuid(), config.getTelegramBotChatId())
            );
        }
    ...
    private InvestBot createBot(SortedSet<CachedCandle> candles, BotState state, BotConfig config, OrderService orderService) {
        ...
        // 2. обработка исторических свечей
        for (CachedCandle historicalCandle : candles) {
            bot.notifyCandle(state.getGroupId(), historicalCandle);
        }

        if (config.getTelegramBotChatId() != null) {
            telegramNotification.ifPresent(n -> n.addObserver(bot.getState().getUuid(), config.getTelegramBotChatId()));
        }


        // 3. подписка на обновления ордеров
